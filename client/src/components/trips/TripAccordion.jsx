...
export default function TripAccordion({ trips, onEdit, onDelete }) {
    if (!trips || trips.length === 0) {
        return <p className="text-muted">No trips added yet. Go and add your first trip!</p>;
    }
                ...
                <Accordion.Item eventKey={trip._id} key={trip._id || index}>
                    ...
                    <Accordion.Body>
                                    ...
                                    {trip.legs.map((leg, legIndex) => (
                                        <Col key={leg._id || legIndex}>
                                            <Card className="accordion-card h-100">
                                                <Card.Body>
                                                    <Card.Title>{leg.city}, {leg.state}</Card.Title>
                                                    <Card.Text>
                                    ...
                                    <Col>
                                        <Card className="accordion-card h-100">
                                            <Card.Body>
                                                ...
                                                <Card.Text>
                                                    <strong>
                                                        ${Number(Object.values(trip.expenses).reduce((sum, val) => sum + val, 0)).toFixed(2)}
                                                    </strong>
                                                </Card.Text>
                                            </Card.Body>
                                        </Card>
                        ...
                        <div className="d-flex justify-content-end gap-2 mt-3">
                            <Button variant="outline-primary" size="sm" onClick={() => onEdit(trip)}>
                                Edit
                            </Button>
                            <Button variant="outline-danger" size="sm" onClick={() => onDelete(trip)}>
                                Delete
                            </Button>
                        </div>
                    </Accordion.Body>
